...
for train_index, test_index in kf.split(dataset, dataset.dataset.data.y):
	wandb.init(project="SANDIRAC_Att_Finish_nci1sepscan", name="SAN_Att"+str(i))
	...
	trainer = pl.Trainer(max_epochs=300,gpus=[0], logger=wandb_logger) #, callbacks=[early_stop_callback])
