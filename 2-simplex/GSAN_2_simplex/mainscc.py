...
#b2_val = b2_sparse.values()
    ...
    dataset_train = SDataset_List(data_list=[sdata_train],root='data_edge/SDataset_List'+ str(seed) + '/train.pt')
    dataset_val = SDataset_List(data_list=[sdata_val],root='data_edge/SDataset_List' +str(seed) + '/val.pt')
    dataset_test = SDataset_List(data_list=[sdata_test],root='data_edge/SDataset_List' + str(seed) + '/test.pt')
    ...
    #     #train model
    #     model = MyNetwork(1, hid_dim, 1, k, dropout, alpha_leaky_relu)
...
#BEST PARAMS FOR EDGE OVER ISUFI 'dropout': 0.49535205074569727, 'alpha_leaky_relu': 0.07212654314370462, 'k': 3, 'hid_dim': 65
    nn = MyNetwork(1,65,1,3, 0.49535205074569727,0.07212654314370462)
    early_stop_callback = EarlyStopping(monitor="val_accuracy", min_delta=0.001, patience=200,verbose=True, mode="max")


    # # # train model
    #eary stopping
    trainer = pl.Trainer(max_epochs=1000, gpus=[1]) #callbacks = [early_stop_callback],deterministic=True,benchmark=True)
